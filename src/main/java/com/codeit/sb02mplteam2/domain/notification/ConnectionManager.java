...
public class ConnectionManager {
  ...
  private static final Long DEFAULT_TIME_UNIT = 1000L * 60; //1000L(=1초) * 60 = 1분 단위
  ...
  public SseEmitter subscribe(Long userId, String lastEventId) {
    ...
    try {
      ...
      Runnable releaseSemaphore = connectionSemaphore::release;

      sseEmitter.onCompletion(() -> {
        removeConnection(userId, "onCompletion");
        releaseSemaphore.run();
      });
      sseEmitter.onTimeout(() -> {
        removeConnection(userId, "onTimeout");
        releaseSemaphore.run();
      });
      sseEmitter.onError(e -> {
        removeConnection(userId, "onError: " + e.getMessage());
        releaseSemaphore.run();
      });
