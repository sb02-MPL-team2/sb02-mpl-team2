...
public interface BinaryContentApi {
  ...
  @ApiResponses(value = {
      ...
      @ApiResponse(
          responseCode = "404", description = "첨부 파일 찾을 수 없음",
  ...
  ResponseEntity<BinaryContentDto> find(
      @Parameter(description = "조회할 첨부 파일 ID") Long binaryContentId
  );
  ...
  ResponseEntity<List<BinaryContentDto>> findAllByIdIn(
    @Parameter(description = "조회할 첨부 파일 ID 목록") List<Long> binaryContentIds
  );
  ...
  ResponseEntity<?> download(
      @Parameter(description = "다운로드할 파일 ID") Long binaryContentId
  );
}
