...
# BLUE가 실행중인지 확인
APP_NAME=mpl-domain-dev
EXIST_BLUE=$(docker-compose -p mpl-domain-dev-blue -f docker-compose.blue.yml ps | grep mpl-domain-dev-blue)
...
# 로컬 컴퓨터에 실행할 것이므로 build를 집어넣음. 만약 AWS에 띄울 거면 뒤에 pull 넣어야 함
docker-compose -p ${APP_NAME}-${START_CONTAINER} -f docker-compose.${START_CONTAINER}.yml build || exit 1
docker-compose -p ${APP_NAME}-${START_CONTAINER} -f docker-compose.${START_CONTAINER}.yml up -d || exit 1
